...
/**
 * Interface for Go coverage data in TIA format
 */
interface GoCoverageData {
  [filePath: string]: {
    path: string;
...
/**
 * Reads and processes Go coverage output for TIA
 */
export class GoCoverageReader {
  ...
  /**
   * Read Go coverage data from TIA per-test coverage files
   */
  async readGoCoverage(): Promise<string[]> {
    const perTestCoverageDir = path.join(this.rootDir, '.tia', 'per-test-coverage');
    
    try {
      if (!fs.existsSync(perTestCoverageDir)) {
        this.logger.debug('No Go per-test coverage directory found');
        return [];
      }
      ...
      if (coverageFiles.length === 0) {
        this.logger.debug('No Go coverage files found');
        return [];
      }
      ...
      for (const coverageFile of coverageFiles) {
        ...
        try {
          const coverageData: GoCoverageData = JSON.parse(
          ...
          Object.keys(coverageData).forEach(sourceFile => {
            ...
            if (hasStatementCoverage && this.isGoSourceFile(sourceFile)) {
              allCoveredFiles.add(this.toRelativePath(sourceFile));
            }
          });

        } catch (error) {
          this.logger.debug(`Failed to process Go coverage file ${coverageFile}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }

      const coveredFiles = Array.from(allCoveredFiles);
      this.logger.debug(`Found ${coveredFiles.length} covered Go files`);
      return coveredFiles;

    } catch (error) {
      this.logger.error(`Failed to read Go coverage: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }

  /**
   * Get detailed coverage statistics from Go coverage data
   */
  async getCoverageStats(): Promise<{
    ...
    try {
      ...
      const aggregatedCoverage = new Map<string, GoCoverageData[string]>();
      ...
      for (const coverageFile of coverageFiles) {
        ...
        try {
          ...
          for (const [sourceFile, coverage] of Object.entries(coverageData)) {
            if (!this.isGoSourceFile(sourceFile)) continue;
      ...
      return {
        ...
        branches: {
          total: 0, // Go coverage doesn't typically track branches in the same way
          covered: 0,
          percentage: 0
        }
      };
      ...
      this.logger.error(`Failed to get Go coverage stats: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }

  /**
   * Check if Go coverage data exists
   */
  hasGoCoverage(): boolean {
    const perTestCoverageDir = path.join(this.rootDir, '.tia', 'per-test-coverage');
    ...
    // Check if any file contains Go coverage schema
    return coverageFiles.some(file => {
      try {
        const filePath = path.join(perTestCoverageDir, file);
        const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        
        // Look for Go coverage schema in any file
        return Object.values(data).some((coverage: any) => 
          coverage._coverageSchema && coverage._coverageSchema.includes('go-coverage')
        );
      } catch {
        return false;
      }
  ...
  /**
   * Check if a file is a Go source file (not test file)
   */
  private isGoSourceFile(filePath: string): boolean {
